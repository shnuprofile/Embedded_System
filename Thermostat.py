...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        # Update lights with correct fading behavior based on temperature
        self.updateLights()
    ...
    def processTempIncButton(self):
        ...
        # Update setPoint and refresh status lights
        self.setPoint += 1
        self.updateLights()
    ...
    def updateLights(self):
        ...
        if(self.current_state.id == 'heat'):
            if(temp < self.setPoint):
                # Actively heating - fade in and out
                redLight.pulse()
            else:
                # At or above setpoint - solid on
                redLight.value = 1
        elif(self.current_state.id == 'cool'):
            if(temp > self.setPoint):
                # Actively cooling - fade in and out
                blueLight.pulse()
            else:
                # At or below setpoint - solid on
                blueLight.value = 1
        # In 'off' state, both lights remain off (already turned off above)
    ##
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                lcd_line_2 = f"T:{floor(self.getFahrenheit()):0.1f}F"
                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"{self.current_state.id} {self.setPoint:0.1f}F"
                altCounter = altCounter + 1
                if(altCounter >= 11):
